...
class ALPFunction implements ALPCallable {
      ...
      @Override
      public int ArgumentSize(){ return (int)declaration.params.size(); }
      @Override
      public Object FunctionCall( Interpreter i , List<Object> args ){
            Environment current = new Environment(closure);

            for( int j=0 ; j<declaration.params.size() ; j++ ) current.define( declaration.params.get(j).getLexme() , args.get(j) ); // defino argumentet e funksionit
                                                                                                                                     //
            try { i.executeBlock( declaration.body , current ); }
            catch( Return r ) { return r.value; }

            return null;
      }
      public String getFunctionName(){ return declaration.name.getLexme(); }
      @Override
      public String toString(){ return "<Funksioni => '" + declaration.name.getLexme() + "'>"; }
}
