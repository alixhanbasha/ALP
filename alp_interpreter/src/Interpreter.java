...
public class Interpreter implements Statement.Visitor<Void>, Expression.Visitor<Object> {
	...
	public Interpreter() {
		new ALPNativeInterface(globals).BindFunctions();
	}
	...
	public Void visitImportStatement(Statement.Import importer) {
		...
		try {
			...
			System.out.println("Moduli per importim nuk u gjet " + io);
		}
		return null;
	}
	...
	public Object visitSetExpression(Expression.Set se) {
		...
		try {
			...
			System.err.println("U shfaq nje problem gjat percaktimit te prones !\t" + ce);
		}
		return val;
	}
	...
	public Object visitBinaryExpression(Expression.Binary be) {
		...
		switch (be.operator.getType()) {
			...
			case SLASH:
				...
				if (Double.parseDouble(String.valueOf(right)) == 0)
					throw new RuntimeError(be.operator, "Pjestimi me 0 nuk mund te kalkulohet!");
				return Double.parseDouble(String.valueOf(left)) / Double.parseDouble(String.valueOf(right));
			case GT:
				checkNumberOperators(be.operator, left, right);
				return Double.parseDouble(String.valueOf(left)) > Double.parseDouble(String.valueOf(right));
			...
			case NOTEQ:
				...
				} else if (left instanceof Boolean && String.valueOf(right).equals("vertet")) {
					return stringify((boolean) left == (boolean) Boolean.parseBoolean(String.valueOf(right)));
