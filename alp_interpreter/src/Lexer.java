...
class Lexer {
	...
	private boolean isDigit(char c) {
		return c >= '0' && c <= '9';
	} /* Custom isDigit method */

	private boolean isAlpha(char c) {
		return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c == '_');
	} /* Return true if char c is alphabetical */
	...
	public void scanToken() {
		...
		switch (c) {
			...
			case ';':
				...
				break;
			case '!':
				addToken(match('=') ? TokenType.NOTEQ : TokenType.BANG);
				break;
			case '=':
			...
			default:
				if (isDigit(c)) { /* Numbers */
					...
					if (type == null)
						type = TokenType.IDENTIFIER; /* If keyword was not found in the map, then its an identifier */
					addToken(type, text);
				} else {
					ALP.throwError(line, "", "Unexpected character -+> " + c);
				}
				break;
		}
	}
